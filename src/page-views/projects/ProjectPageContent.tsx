    ...
    <div className={styles.shell}>
      ...
      <div className={styles.rightColumn}><div className={contextStyles.tabBody}>
        ...
        {isContextOpen ? <SectionContainer title="Project Context" tone="context" hideTitle><div className={styles.tabBody}>
          {activeTab === "details" && selectedProject ? <div className={styles.detailGrid}><div className={styles.metaCard}><p className={styles.metaLabel}>Project Name</p><p className={styles.metaValue}>{selectedProject.name}</p></div><div className={styles.metaCard}><p className={styles.metaLabel}>Team</p><p className={styles.metaValue}>{selectedProject.teamName ?? "No Team"}</p></div><div className={styles.metaCard}><p className={styles.metaLabel}>Visibility</p><p className={styles.metaValue}>{selectedProject.visibilityScope}</p></div><div className={styles.metaCard}><p className={styles.metaLabel}>Derived Project Progress</p><p className={styles.metaValue}>{formatProgress(selectedProject.progress)}</p></div></div> : null}
          {activeTab === "details" && !selectedProject ? <p className={styles.emptyText}>Select a project to view details.</p> : null}
          {activeTab === "milestones" && selectedProject ? <div className={styles.replyList}>{selectedProject.milestones.map((milestone) => { const expanded = expandedMilestoneIds.has(milestone.id); return <div key={milestone.id} className={styles.contextCard}><button type="button" className={styles.accordionButton} onClick={() => toggleMilestone(milestone.id)}><span>{milestone.name}</span><span>{expanded ? "Hide" : "Show"}</span></button><div className={styles.milestoneHeader}><p className={styles.progressText}>Progress: {formatProgress(milestone.progress)}</p><Button label="Add Item" variant="secondary" onClick={() => openCreate(milestone.id)} /></div>{expanded ? <><p className={styles.itemMeta}>Description: {milestone.description ?? "n/a"}</p><div className={styles.itemList}>{milestone.items.length === 0 ? <p className={styles.emptyText}>No items in this milestone.</p> : milestone.items.map((item) => <button key={item.id} type="button" className={styles.itemRowButton} onClick={() => openDetail(milestone.id, item.id)}><div className={styles.itemRow}><p className={styles.itemTitle}>{item.title}</p><p className={styles.itemMeta}>Owner: {item.ownerName} - <span className={styles.stateBadge}>{item.state}</span> - Progress: {formatProgress(item.stepProgress)} - Due: {formatDueAt(item.dueAt)}</p></div></button>)}</div></> : null}</div> })}</div> : null}
          {activeTab === "milestones" && !selectedProject ? <p className={styles.emptyText}>Select a project to view milestones.</p> : null}
